(async function bootVajehYar(){
  ...
  try {
    await load(`./ielts-bank-v2.5.js?release=${RELEASE}`);
    await load(`./app-v2.5.js?release=${RELEASE}`);
  } catch (error) {
    const message=document.createElement('div');message.style.cssText='position:fixed;inset:20px;z-index:9999;padding:20px;border-radius:16px;background:#fff1f2;color:#9f1239;font-family:Arial,sans-serif';message.textContent='VajehYar could not load the latest IELTS bank. Refresh the page once.';document.body.appendChild(message);
  }
})();
